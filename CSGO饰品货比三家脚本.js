...
function addC5(c5URL, li, itemName) {
    GM_xmlhttpRequest({
        ...
        onload: function (response) {
            ...
            if (!hasNextPage) $(li).html('<span class="myTitle">C5：</span><span style="color: #FF0000">查找不到数据！</span>');  //若没有下一页则可以判断没有该物品的数据
            else {
...
function addIGXE(igxeURL, li, itemName) {
    GM_xmlhttpRequest({
        ...
        onload: function (response) {
            ...
            if (!hasNextPage) $(li).html('<span class="myTitle">IGXE：</span><span style="color: #FF0000">查找不到数据！</span>');  //若没有下一页则可以判断没有该物品的数据
...
function addBUFF(buffURL, li, itemName) {
    GM_xmlhttpRequest({
        ...
        onload: function(response) {
            ...
            if (cur_page >= total_pages) $(li).html('<span class="myTitle">BUFF：</span><span style="color: #FF0000">查找不到数据！</span>');
            else {
                var next_page = cur_page + 1;
...
function addV5(v5URL, li, itemName) {
    GM_xmlhttpRequest({
        ...
        onload: function (response) {
            ...
            if (!hasNextPage) $(li).html('<span class="myTitle">V5FOX：</span><span style="color: #FF0000">查找不到数据！</span>');
            else {
